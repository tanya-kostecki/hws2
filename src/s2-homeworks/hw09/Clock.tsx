...
function Clock() {
    ...
    const editedStringTime = `${formatTime(date.getHours())}:${formatTime(
      date.getMinutes()
    )}:${formatTime(date.getSeconds())}`;

    const editedStringDate = `${formatTime(date.getDate())}.${formatTime(
      date.getMonth() + 1
    )}.${formatTime(date.getFullYear())}`;

    const stringDate = editedStringDate || <br />;

    const stringTime = editedStringTime || <br />; // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
    const dayOptions: Intl.DateTimeFormatOptions = { weekday: "long" };

    const stringDay = date.toLocaleDateString("en-US", dayOptions) || <br />;

    const monthOptions: Intl.DateTimeFormatOptions = { month: "long" }
    const stringMonth = new Intl.DateTimeFormat('en-US', monthOptions).format(date) || <br />; // пишут студенты
